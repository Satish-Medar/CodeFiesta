    ...
    <div className="min-h-screen bg-white dark:bg-zinc-950 text-slate-900 dark:text-white px-4 py-20">
      <div className="max-w-4xl mx-auto space-y-8">
        <header className="rounded-2xl border border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900 p-10 shadow-sm">
          ...
          {roleNotSet && (
            <div className="mt-4 rounded-lg bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-700 p-4">
              <p className="text-sm font-medium text-yellow-800 dark:text-yellow-100">
                Your role isn’t set yet. Pick a role to get started.
              </p>
              <div className="mt-3 flex flex-wrap gap-3">
                <button
                  onClick={() =>
                    setUserRole("organizer").then((updated) => {
                      if (updated) {
                        setRole(updated.role || "organizer");
                        setAppUser(updated);
                      }
                    })
                  }
                ...
                <button
                  onClick={() =>
                    setUserRole("volunteer").then((updated) => {
                      if (updated) {
                        setRole(updated.role || "volunteer");
                        setAppUser(updated);
                      }
                    })
                  }
                  className="rounded-md border border-slate-300 bg-white text-slate-900 px-4 py-2 text-sm hover:bg-slate-50 dark:border-zinc-700 dark:bg-zinc-950 dark:text-white dark:hover:bg-zinc-900"
                >
                  I’m a Volunteer
                </button>
              </div>
